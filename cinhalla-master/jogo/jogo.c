...
/*int** carregarMapa(const char* nome_arquivo, int* linhas, int* colunas){
...
void liberaMapa(int** mapa, int linhas){
    int i;
    for(i = 0; i < linhas; i++){
        free(mapa[i]);
    }
...
int main(void){
    /*int linhas, colunas;
    int **mapaMatriz = carregaMapa("mapa.txt", &linhas, &colunas);*/
    ...
    while(!sair){   
        ...
        if(1){
            if((posyPerso>=365 && (posxPerso<20 || posxPerso>=830)) || (posxPerso>=830) || (posyPerso>365 && posxPerso<420)|| (posyPerso>265 && posxPerso>=450)){ // CAI
                posyPerso = cair (tecla, inverte, regiaoxPerso, regiaoyPerso, larguraPerso, alturaPerso, posxPerso, posyPerso, lado);
            }
            if(posxPerso>385 && posxPerso<430 && posyPerso == 365){ // PORTAL DEBAIXO
                posxPerso = 465;
                posyPerso = 265;
            }
            if(posxPerso>=420 && posxPerso<460 && posyPerso == 265){ // PORTAL DE CIMA
                posxPerso = 380;
                posyPerso = 365;
            }
            if(posyPerso>600){
